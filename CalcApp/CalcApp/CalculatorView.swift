...
struct CalculatorView: View {
    ...
    func handleTap(_ btn: String) {
        ...
        case "÷", "×", "−", "+":
            firstNum = Double(display)
            currentOp = btn
            fullExpression = "\(display) \(btn)"
            freshInput = true

        case "=":
            guard let op = currentOp, let first = firstNum,
...
struct CalcButton: View {
    ...
    var bgColor: AnyView {
        if isOperator {
            ...
            return AnyView(Color.white.opacity(0.18))
        } else {
    ...
    var body: some View {
        Button(action: action) {
            Text(label)
                ...
                .foregroundColor(isOperator ? .white : .white)
                .frame(width: isWide ? 168 : 78, height: 78)
                .background(bgColor)
                .clipShape(RoundedRectangle(cornerRadius: 24))
                .shadow(
                    color: isOperator ? Color(hex: "#a855f7").opacity(0.55) : .black.opacity(0.25),
                    radius: 10, y: 5
                )
        }
        .buttonStyle(.plain)
